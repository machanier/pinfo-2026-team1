 ...
 * 
 * @ApplicationScoped
 * public class toremoveModerationService {
 * 
 * private static final Logger LOG =
 * Logger.getLogger(toremoveModerationService.class);
 * 
 * @Inject
 ...
 * try {
 * // Appel à OpenAI à travers LangChain4j
 * String raw = moderationAiService.moderateEvent(
 * event.title,
 * event.description != null ? event.description : "");
 * 
 * AiModerationResult result = objectMapper.readValue(raw,
 * AiModerationResult.class);
 * 
 ...
 * LOG.errorf("AI screening failed for eventId=%s: %s", event.eventId,
 * e.getMessage());
